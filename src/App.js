...
class App extends Component {
  ...
  componentDidMount = () => {
    this.intervalId = setInterval(this.status, 1000)
  }
  ...
  getThumbnail = thumbnailUrl => {
    ...
    const imgListItem = imagesList.filter(
      each => each.thumbnailUrl === thumbnailUrl,
    )
    const {imageUrl} = imgListItem[0]
  ...
  render() {
    const {score, timer, isTrue, imgUrl, category} = this.state
          ...
          <ul className="nav-items-container">
            ...
            <li className="timer-container">
              ...
              <div className="time-container">
                <p className="count1">{timer} sec</p>
              </div>
            </li>
          </ul>
        </nav>
              ...
              <ul className="tabs-container">
                {tabsList.map(each => (
                  <TabItem
                    details={each}
                    key={each.tabId}
                    onChangeTab={this.changeTabItem}
                    categoryItem={category}
                  />
                ))}
              </ul>
              <ul className="thumgnail-container">
                {thumbnailList.map(each => (
                  <ThumbnailItem
                    details={each}
                    key={each.id}
                    onClickThumbnail={this.getThumbnail}
                  />
                ))}
              </ul>
            </div>
